...
<div class="relative">
    <div
        ...
        tabindex="-1"
        style="background: linear-gradient(90deg, {colour1} 0%, {colour1} {home *
            100}%, {colour2} {home *
            100}%, {colour2} 100%);); border: 1px solid {colour}; transition: border-color 0.15s"
    ...
    <div class="flex absolute right-2 lg:right-auto top-2 lg:block lg:relative lg:-translate-y-8">
        <div
            class="flex items-center lg:absolute h-full top-0 pr-2 text-white-{home >= away
                ? 900
                : 800} pointer-events-none"
            style="right: {100 - home * 100}%"
        >
            <p class=" font-normal">{(home * 100).toFixed(0)}%</p>
        </div>
        <p class="lg:hidden text-white-800">/</p>
        <div
            class="flex items-center lg:absolute h-full top-0 pl-2 text-white-{home >= away
                ? 800
                : 900} pointer-events-none"
            style="left: {100 - away * 100}%"
        >
