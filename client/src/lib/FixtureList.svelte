    ...
    $: averageXGA =
        $leagueShotsConceded == null
            ...
            : Array.from($leagueShotsConceded.data.entries()).reduce(
                  (a, [team, shots]) => a + shots.xG / $dataManager.teams.get(team).games,
                  0,
              ) / $leagueShotsConceded.data.size;
</script>

<AveragePointer
    percentage={(averageXGA / xgMax) * 100}
    text={`League average ${averageXGA.toFixed(2)} xGA per 90`}
/>
{#each opponents.teams as team, i}
    <FixtureSummary {team} side={opponents.sides[i]} />
{/each}
