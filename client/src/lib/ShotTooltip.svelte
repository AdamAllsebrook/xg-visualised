...
<div class='inline-block'>
    <p>{data.minute}</p>
    <p>{data.situation.replace(/([A-Z])/g, ' $1').trim()}</p>
    <p>{data.player_assisted || '-'}</p>
    <p>{data.shotType.replace(/([A-Z])/g, ' $1').trim()}</p>
    <p>{data.result.replace(/([A-Z])/g, ' $1').trim().replace('Shots', 'Shot')}</p>
    <p>{data.xG.toFixed(2)}</p>
</div>
